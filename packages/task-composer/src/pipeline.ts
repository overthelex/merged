...
export interface ComposeTaskInputs {
  ...
  repoPath: string;
  level: Level;
  /** Override agent-level configs. */
  scout?: ScoutConfig;
  composer?: ComposerConfig;
  calibrator?: CalibratorConfig;
  scan?: ScanOptions;
  /** Max Composer↔Calibrator iterations. Default 2 (so up to 3 Composer calls). */
  maxRevisions?: number;
}
...
export interface ComposeTaskResult {
  ...
  surfaces: Surface[];
  meta: RepoMeta;
  /** Every verdict we collected, in order. Last one is `ok: true`. */
  verdicts: CalibratorVerdict[];
}

/**
 * End-to-end pipeline: scan repo → Scout (parallel) → Composer → Calibrator.
 * Composer drafts again on each Calibrator rejection, up to `maxRevisions`.
...
export async function composeTask(inputs: ComposeTaskInputs): Promise<ComposeTaskResult> {
  ...
  for (let round = 0; round <= maxRevisions; round++) {
    ...
    const { verdict, spec } = await runCalibrator({ draft, level: inputs.level }, inputs.calibrator);
  ...
  throw new Error(
    `calibrator rejected all ${verdicts.length} drafts · last notes: ${[
      ...last.schema_errors,
      ...last.coherence_notes,
    ]
      .slice(0, 5)
      .join(' | ')}`,
  );
