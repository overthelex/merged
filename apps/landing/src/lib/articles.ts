...
1. **Staff-інженер компанії-замовника.** Він задав рубрику і формат задачі. Саму ж постановку ТЗ генерує і вичитує наш 4-агентний Bedrock-пайплайн (Scout → Composer → Calibrator → Verifier, усі ключові — на **Claude Opus 4.7**, найновішій моделі Bedrock-серії Opus) — перш ніж вона взагалі доходить до вас. Деталі — у статті "[Як ми генеруємо задачі](/blog/bedrock-pipeline-verifier)".
...
Наш pipeline виглядає так (усі три етапи — **Claude Opus 4.7 на AWS Bedrock**, найновіша модель Bedrock-серії Opus):
...
Це половина історії. **Другу половину** — як ми генеруємо саму задачу, яку вам пропонують, і чому там теж вʼязаний 4-агентний Bedrock-пайплайн з новим агентом Verifier, що перевіряє постановку ТЗ — описано в статті "[Як ми генеруємо задачі](/blog/bedrock-pipeline-verifier)". Обидва пайплайни живуть на одній моделі — Opus 4.7.
...
`,
  ...
  {
    slug: 'bedrock-pipeline-verifier',
    title: 'Як ми генеруємо задачі: 4-агентний Bedrock-пайплайн і новий Verifier',
    punchline:
      'Кожна задача для кандидата проходить через чотири Bedrock-агенти: Scout, Composer, Calibrator і новий Verifier, який перечитує ТЗ очима кандидата та оцінювача. Усі чотири — на Claude Opus 4.7, найновішій моделі Bedrock-серії Opus.',
    category: 'research',
    tags: ['bedrock', 'task-composer', 'claude-opus-4-7', 'verifier', 'pipeline'],
    readTime: '6 хв читання',
    publishedAt: '2026-04-19',
    content: `# Як ми генеруємо задачі: 4-агентний Bedrock-пайплайн і новий Verifier
...
- Чи rubric criteria можна оцінити обʼєктивно? \`source: auto\` — тільки там, де справді можна CI-виміряти?
- Чи \`time_limit_min\` узгоджений з реальним обсягом?
- Scope leak: чи не вимагається щось поза level (architecture для junior) або поза стеком?
...
- Opus 4.6 інколи "коректно" пропускав ТЗ з rubric, де всі критерії були \`source: llm\` — без жодного автоматично вимірюваного. Verifier на 4.6 не вважав це проблемою.
- Opus 4.7 ці випадки ловить. Припущення: краща калібровка на довгих структурованих output — Verifier має тримати в голові task.yaml + ASSIGNMENT.md + meta одночасно.
...
Verifier ловить проблеми ТЗ. **Виконання ТЗ** кандидатом оцінює окремий пайплайн — LLM-суддя з трьох етапів (нормалізація → per-criterion → агрегація, теж на Opus 4.7). Про нього — окрема стаття: "[Рубрика як артефакт](/blog/rubryka-llm-suddya)".
...
- Усі чотири агенти — в одному npm-пакеті \`@merged/task-composer\`, без черг: Server Action блокується на 60–90 секунд.
- Bedrock-реджн: \`eu-central-1\` (Франкфурт) — дані не виходять за межі ЄС.
- Бюджет токенів: ~180k на Scout/Composer-чанк, 8192 max output для Composer, 2048 для Calibrator/Verifier.
- Retry на 429/5xx з jitter-exponential backoff (3 спроби).
- Env-змінні для моделей: \`BEDROCK_SCOUT_MODEL_ID\`, \`BEDROCK_COMPOSER_MODEL_ID\`, \`BEDROCK_CALIBRATOR_MODEL_ID\`, \`BEDROCK_VERIFIER_MODEL_ID\` — будь-який можна перевизначити у staging/dev.
